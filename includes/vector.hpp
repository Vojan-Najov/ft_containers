...
class Vector_base {
 public:
  ...
  Vector_base(const allocator_type& alloc)
      : allocator(alloc), start(0), finish(0), end_of_storage(0) {}
  Vector_base(size_type n, const allocator_type& alloc) : allocator(alloc) {
    start = allocate(n);
    finish = start;
    end_of_storage = start + n;
  }
  ~Vector_base(void) { deallocate(start, end_of_storage - start); }
 ...
 protected:
  ...
  void deallocate(pointer p, size_type n) {
    if (p) allocator.deallocate(p, n);
  }
  void construct(pointer p, T const& value) { allocator.construct(p, value); }
...
class vector : protected Vector_base<T, Allocator> {
 ...
 public:
  ...
  explicit vector(size_type n, value_type const& value = T(),
 ...
 protected:
  ...
  template <typename InputIterator>
  pointer _uninitialized_copy(InputIterator first, InputIterator last,
                              pointer ptr);
  ...
  template <typename InputIterator>
  void _initialize_aux(InputIterator first, InputIterator last, false_type);
  ...
  template <typename ForwardIterator>
  void _assign_aux(ForwardIterator first, ForwardIterator last,
                   std::forward_iterator_tag);
  ...
  template <typename Integral>
  void _insert_dispatch(iterator position, Integral n, Integral value,
                        true_type);
...
typename vector<T, A>::const_reference vector<T, A>::operator[](
    typename vector<T, A>::size_type n) const {
  return *(start + n);
}
...
template <typename T, typename A>
typename vector<T, A>::reference vector<T, A>::at(
    typename vector<T, A>::size_type n) {
  if (n >= size()) throw std::out_of_range("Index is out of range");
  return *(start + n);
}

template <typename T, typename A>
typename vector<T, A>::const_reference vector<T, A>::at(
    typename vector<T, A>::size_type n) const {
  if (n >= size()) throw std::out_of_range("Index is out of range");
  return *(start + n);
}
...
void vector<T, A>::insert(typename vector<T, A>::iterator position,
                          typename vector<T, A>::size_type n,
                          typename vector<T, A>::value_type const& value) {
  if (n != 0) _insert_fill(position.base(), n, value);
}
...
typename vector<T, A>::iterator vector<T, A>::erase(
    typename vector<T, A>::iterator position) {
  pointer posptr = position.base();
  if (posptr + 1 != finish) ft::copy(posptr + 1, finish, posptr);
  --finish;
  destroy(finish);
  return position;
}
...
template <typename T, typename A>
typename vector<T, A>::pointer vector<T, A>::_uninitialized_fill_n(
    typename vector<T, A>::pointer ptr, typename vector<T, A>::size_type n,
    typename vector<T, A>::value_type const& value) {
  pointer tmp = ptr;
  while (n > 0) {
    try {
...
template <typename T, typename A>
template <typename InputIterator>
typename vector<T, A>::pointer vector<T, A>::_uninitialized_copy(
    InputIterator first, InputIterator last,
...
template <typename T, typename A>
template <typename InputIterator>
typename vector<T, A>::pointer vector<T, A>::_allocate_and_copy(
    typename vector<T, A>::size_type n, InputIterator first,
    InputIterator last) {
...
void vector<T, A>::_initialize_aux(Integer n, Integer value, ft::true_type) {
  start = allocate(n);
  end_of_storage = start + n;
  finish = _uninitialized_fill_n(start, static_cast<size_type>(n),
                                 static_cast<value_type>(value));
}

template <typename T, typename A>
template <typename InputIterator>
void vector<T, A>::_initialize_aux(InputIterator first, InputIterator last,
                                   ft::false_type) {
  _range_initialize(
      first, last,
...
template <typename T, typename A>
template <typename InputIterator>
void vector<T, A>::_range_initialize(InputIterator first, InputIterator last,
                                     std::input_iterator_tag) {
  while (first != last) {
    push_back(*first);
    ++first;
...
template <typename T, typename A>
template <typename Integral>
void vector<T, A>::_assign_dispatch(Integral n, Integral value, ft::true_type) {
  _fill_assign(static_cast<size_type>(n), static_cast<value_type>(value));
}

template <typename T, typename A>
template <typename InputIterator>
void vector<T, A>::_assign_dispatch(InputIterator first, InputIterator last,
                                    ft::false_type) {
...
template <typename T, typename A>
template <typename InputIterator>
void vector<T, A>::_assign_aux(InputIterator first, InputIterator last,
                               std::input_iterator_tag) {
  iterator cur = begin();
  for (; first != last && cur != end(); ++first, ++cur) *cur = *first;
  if (first == last)
    erase(cur, end());
  else
...
template <typename T, typename A>
template <typename ForwardIterator>
void vector<T, A>::_assign_aux(ForwardIterator first, ForwardIterator last,
                               std::forward_iterator_tag) {
...
void vector<T, A>::_insert_fill(typename vector<T, A>::pointer position,
  ...
  if (static_cast<size_type>(end_of_storage - finish) >= n) {
    ...
    if (elements_after_pos > n) {
      ...
      finish = _uninitialized_fill_n(finish, n - elements_after_pos, val_copy);
    ...
    size_type len =
        (2 * capacity() < n + old_size ? n + old_size : 2 * capacity());
    pointer new_start = allocate(len);
    pointer new_finish = new_start;
    pointer except_ptr = new_start;
    try {
...
void vector<T, A>::_insert_dispatch(typename vector<T, A>::iterator position,
                                    Integral n, Integral value, ft::true_type) {
  if (n != 0)
    _insert_fill(position.base(), static_cast<size_type>(n),
                 static_cast<value_type>(value));
}
...
template <typename T, typename Allocator>
inline bool operator==(vector<T, Allocator> const& lhs,
