...
template <typename Iterator>
inline typename iterator_traits<Iterator>::difference_type _distance(
...
template <typename Iterator>
inline void _advance(Iterator& it,
...
inline void advance(Iterator& it, Dist n) {
...
template <typename Iter>
reverse_iterator<Iter>::reverse_iterator(void) : current() {}
...
template <typename Iter>
typename reverse_iterator<Iter>::iterator_type reverse_iterator<Iter>::base(
    void) const {
  return current;
}
...
template <typename Iter>
template <typename Tp>
typename reverse_iterator<Iter>::pointer reverse_iterator<Iter>::_s_to_pointer(
    Tp it) {
  return it.operator->();
}
...
template <typename Iter>
inline bool operator==(reverse_iterator<Iter> const& lhs,
...
inline bool operator==(reverse_iterator<IterL> const& lhs,
...
inline typename reverse_iterator<Iter1>::difference_type operator-(
    reverse_iterator<Iter1> const& lhs, reverse_iterator<Iter2> const& rhs) {
  return rhs.base() - lhs.base();
}
...
template <typename InputIterator1, typename InputIterator2>
inline bool lexicographical_compare(InputIterator1 first1, InputIterator2 last1,
                                    InputIterator2 first2,
...
inline void swap(T (&a)[N], T (&b)[N]) {
  for (size_t i = 0; i < N; ++i) ft::swap(a[i], b[i]);
}
...
same syntax as a regular function call, and therefore its type can be used as
template parameter when a generic function type is expected.
*/
